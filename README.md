...
- **Other Skills (Strategist, Architect, Guardian, Auditor)**: 🟢 **Text-Only.** These skills only process the generated Markdown specifications or JSON node data. They do NOT require any external API connections, making them 100% portable for users without a Figma setup.

## 🔄 Recommended Workflow

For the best results, use these skills in the following order to create a high-fidelity, strategy-aligned design loop:

1.  **Strategize the Feature**: Use the **[Product Spec Strategist](./skills/product_spec_strategist)** to turn raw requirements into an actionable design brief with 3 UX flow options.
2.  **Map the Design System**: Use the **[Design System Spec Writer](./skills/design_system_spec_writer)** with a Figma URL to generate a `ui_spec_artifact`.
3.  **Architect the UI**: Pass the chosen UX flow and the `ui_spec_artifact` into the **[UI Design Architect](./skills/ui_design_architect)** to generate an execution prompt.
4.  **Build the Component**: Use the Architect's output to generate the UI using your preferred tool (e.g., Stitch, v0, or manual code).
5.  **Audit for Friction**: Run the **[Heuristic UI Auditor](./skills/heuristic_ui_auditor)** to identify accessibility and usability issues.
6.  **Strategic Governance**: Finally, use the **[UX Strategy Guardian](./skills/ux_strategy_guardian)** to ensure the build aligns perfectly with your high-level principles.
